...
class DiscussComments extends Model
{
    //
    protected  $table = 'discuss_comments';

    private static function table(){
        return 'discuss_comments';
    }
    /**
    ...
    public static function specialColumns2NextRowsAsc($column, $post, $column2, $post2)
    {
        //Utility::specialColumns(self::table(),$column, $post);
        return static::where('status', '=',Utility::STATUS_ACTIVE)
            ->where($column, '=',$post)->where($column2, '>',$post2)->orderBy('id','ASC')->get();
