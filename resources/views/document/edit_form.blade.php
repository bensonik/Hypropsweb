        ...
        <div class="row clearfix">
            ...
            <div class="col-sm-8">
                ...
                <div class="form-group">
                    <div class="form-line">
                        <input type="text" class="form-control" value="{{$edit->doc_name}}" name="document_name" placeholder="Document Name">
                    </div>
                </div>
            </div>
        ...
        @if(!empty($edit->userAccess))
            @foreach($edit->userAccess as $user)
                <div class="row clearfix" id="remove_user{{$user->id}}">
                    <div class="col-sm-8" id="">
                        <div class="form-group">
                            <div class="form-line">
                                ...
                                <input type="hidden" value="{{$user->id}}" class="user_class_edit" name="user" id="user_edit" />
                            </div>
                        </div>
                    ...
                    <div class="col-sm-3">
                        <button type="button" onclick="deleteSingleItemWithParam('{{$edit->id}}','{{$user->id}}','reload_data','<?php echo url('document'); ?>',
                                '<?php echo url('remove_document_user'); ?>','<?php echo csrf_token(); ?>','remove_user{{$user->id}}');" class="btn btn-danger">
