...
@section('content')
        ...
        <div class="modal-dialog " role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="defaultModalLabel">New Sales Team</h4>
                </div>
                                ...
                                <div class="col-sm-4" id="">
                                    <div class="form-group">
                                        <div class="form-line">
                                            ...
                                            <input type="hidden" class="user_class" name="users" id="user" />
                                        </div>
                                    </div>
                ...
                <div class="modal-footer">
                    <button onclick="submitData('createModal','createMainForm','<?php echo url('create_crm_sales_team'); ?>','reload_data',
                            '<?php echo url('crm_sales_team'); ?>','<?php echo csrf_token(); ?>','user_class')" type="button" class="btn btn-info waves-effect">
                        SAVE
                    </button>
    ...
    <div class="modal fade" id="editModal" tabindex="-1" role="dialog">
        <div class="modal-dialog " role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button"  onclick="submitData('editModal','editMainForm','<?php echo url('edit_crm_sales_team'); ?>','reload_data',
                            '<?php echo url('crm_sales_team'); ?>','<?php echo csrf_token(); ?>','user_class_edit')"
            ...
            <div class="card">
                <div class="header">
                    ...
                    <ul class="header-dropdown m-r--5">
                        ...
                        <li>
                            <button type="button" onclick="deleteItems('kid_checkbox','reload_data','<?php echo url('crm_sales_team'); ?>',
                                    '<?php echo url('delete_crm_sales_team'); ?>','<?php echo csrf_token(); ?>');" class="btn btn-danger">
                ...
                <div class="container">
                    <div class="col-sm-6 ">
                        <div class="form-group">
                            <div class="form-line">
                                <input type="text" id="search_sales_team" class="form-control"
                                       onkeyup="searchItem('search_sales_team','reload_data','<?php echo url('search_crm_sales_team') ?>','{{url('crm_sales_team')}}','<?php echo csrf_token(); ?>')"
                                       name="search_user" placeholder="Search sales team" >
                            </div>
                        </div>
                    </div>
                </div><hr/>
                    ...
                    <table class="table table-bordered table-hover table-striped" id="main_table">
                        ...
                        @foreach($mainData as $data)
                            <tr>
                                ...
                                <td>
                                    <a style="cursor: pointer;" onclick="editForm('{{$data->id}}','edit_content','<?php echo url('edit_crm_sales_team_form') ?>','<?php echo csrf_token(); ?>')"><i class="fa fa-pencil-square-o fa-2x"></i></a>
                                </td>
                                @else
                                <td></td>
                                @endif
    ...
    <script>
        ...
        function submitData(formModal,formId,submitUrl,reload_id,reloadUrl,token,userClass){
            var form_get = $('#'+formId);
